...
UDPNetwork::UDPNetwork(uint16_t port) {
    auto *i = new Impl();
    i->sock = create_socket_bind(port);
    if (i->sock < 0) throw std::runtime_error("failed to create/bind UDP socket");
    i->running = false;
    this->impl = i;
}
...
void UDPNetwork::stop_receive() {
    ...
    if (impl->running) {
        impl->running = false;
        // interrupt recvby sending empty datagram to self
        sockaddr_in addr{};
        addr.sin_family = AF_INET;
...
std::optional<std::string> UDPNetwork::send_request_wait_response(const std::string &to_addr, const std::string &msg, int timeout_ms) {
    ...
    // we got some incoming datagram; return it
...
// private impl pointer
// defined here to avoid header exposing Impl
struct UDPNetwork::Impl;
UDPNetwork::Impl* UDPNetwork::impl = nullptr;
